 ...
 */
async function updateGeminiKey(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
...
/**
 * DELETE /api/gemini/[id]
 * Delete Gemini API key
 */
async function deleteGeminiKey(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    const { id } = await params;
    const keyId = parseInt(id);
    ...
    await db.request()
      .input('id', sql.Int, keyId)
      .query(`DELETE FROM [dbo].[gemini_keys] WHERE [id] = @id`);
...
export const PUT = requireAdmin(updateGeminiKey);
export const DELETE = requireAdmin(deleteGeminiKey);
